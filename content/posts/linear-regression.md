...
title: "Estimating probabilities of data"
date: "2025-10-30"
...
Given observations $\mathcal{D}=\lbrace x_1,...,x_n \rbrace $ and for a family of probability densities $p(x|\theta)$ parametrized by $\theta$, the likelihood of the data is given by $p(\mathcal{D}|\theta)$. The MLE principle is to find a $\hat{\theta}$ that maximzes the likelihood of data, $p(\mathcal{D}|\theta)$
...
$$\hat{\theta}_{MLE} = \arg\min_{\theta} -\log p(\mathcal{D}| \theta)$$
...
$$p(\mathcal{D}|\theta) = {{n_H + n_T}\choose {n_H}} \theta^{n_H} (1 - \theta)^{n_T}$$
...
- $p(\mathcal{D}|\theta)$ is the **likelihood** of the data given the parameter(s) $\theta$
- $p(\theta|\mathcal{D})$ is the **posterior** distribution over the parameter(s) **after** we have observerd the data.
...
**MAP Principle:** Find $\hat\theta$ that maximizes the posterior distribution $p(\theta|\mathcal{D})$

$$\hat{\theta}_{MAP} = \arg\max_{\theta} p(\theta|\mathcal{D})$$

$$\hat{\theta}_{MAP} = \arg\max_{\theta} \frac{p(\mathcal{D}|\theta)p(\theta)}{p(\mathcal{D})}$$

Here $p(\mathcal{D})$ is independent of $\theta$ so we get:

$$\hat{\theta}_{MAP} = \arg\min_{\theta} -\log p(\mathcal{D}|\theta) - \log p(\theta)$$


For the coin toss example, we assumed a Beta distribution, so we get the following:

$$p(\theta|\mathcal{D})\propto p(\mathcal{D}|\theta)p(\theta) \propto \theta^{n_H + \alpha - 1}(1 - \theta)^{n_T + \beta - 1}$$
...
Focusing solely on some static of the posterior distribution (such as the parameter $\theta^{\star}$ that maximizes the posterior) leads to loss of informtaion, which can be critical in a system that uses the prediction $p(x|\theta^{\star})$ to make decisions. There is much more information  in the $p(\theta|\mathcal{D})$ and we are simply computing mode and throwing all other information away. A true Bayesian approach is to use the posterior predictive distribution directly to make prediction about the label $\mathcal{Y}$ of a test sample with features $\mathcal{X}$:

$$p(\mathcal{Y|\mathcal{D, X}}) = \int_{\theta}p(\mathcal{Y}, \theta|\mathcal{D}, \mathcal{X})d\theta = \int_{\theta}p(\mathcal{Y}| \theta,\mathcal{D}, \mathcal{X})p(\theta|\mathcal{D})d\theta$$
...
$$p(H|\mathcal{D}) = \int_\theta p(H, \theta| \mathcal{D})d\theta$$

$$p(H|\mathcal{D}) = \int_\theta p(H |\theta, \mathcal{D})p(\theta|\mathcal{D})d\theta$$

$$p(H|\mathcal{D}) = \int_\theta \theta(\theta|\mathcal{D})d\theta$$

$$p(H|\mathcal{D}) = E[\theta|\mathcal{D}]$$
$$=\frac{n_H + \alpha}{n_H + \alpha + n_T + \beta}$$

Here, we assumed our data is drawn from Binomial Distribution so we could use the fact that $p(H|\mathcal{D}, \theta) = p(H|\theta)$, in general this would not hold
...
Since the likelihood function $p(\mathcal{D}|\theta)$ is the predictive distribution of the data given model parameters, we need to marginalize out the latent variables.

$$p(\mathcal{D}|\theta) = \int p(\mathcal{D}|z, \theta)p(z)dz$$
...
One challenge we have in this latent-variable model is that likelihood $p(\mathcal{D}|\theta)$ requires marginalization of the latent variables. Except when we choose a conjugate prior $p(z)$, the marginalization is not analytically tractable and we need to resort to approximations.

Similar to parameter posterios, we can compute a posterior on the latent variables according to 

$$p(z|\mathcal{D}) = \frac{p(\mathcal{D}|z)p(z)}{p(\mathcal{D})},  p(\mathcal{D}|z) = \int p(\mathcal{D}|z, \theta)p(\theta)d\theta$$
